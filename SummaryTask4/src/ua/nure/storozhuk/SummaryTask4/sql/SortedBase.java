...
public class SortedBase extends DataBaseMain {
	...
	public List<AverageMark> getAvMark(String order){
		List<AverageMark> marks = new LinkedList<AverageMark>();
		if(order == null || order.isEmpty()) {
			order = "asc";
		}
		try {
			rs = st.executeQuery("SELECT student_id, (SELECT login from users where users.id = journal.student_id) AS login, AVG(mark) as mark"
					+ " from journal GROUP BY student_id ORDER BY mark "+order);
			while(rs.next()) {
				marks.add(new AverageMark(rs.getInt(1), rs.getFloat(3), rs.getString(2)));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
