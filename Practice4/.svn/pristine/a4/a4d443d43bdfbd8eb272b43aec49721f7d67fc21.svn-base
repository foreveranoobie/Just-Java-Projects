	...
	public static void main(String[] args) {
		...
		while (m.find()) {
			for (int charNum = m.start(); charNum < m.end(); charNum++) {
				replacement = String.valueOf(result.charAt(charNum)).toLowerCase(Locale.getDefault());
				if (String.valueOf(result.charAt(charNum)).equals(replacement)) {
					replacement = String.valueOf(result.charAt(charNum)).toUpperCase(Locale.getDefault());
